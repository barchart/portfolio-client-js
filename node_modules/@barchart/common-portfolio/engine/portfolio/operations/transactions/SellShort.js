...
module.exports = (() => {
	...
	/**
	 * Creates a {@link TransactionType.SELL_SHORT} transaction.
	 *
	 * @public
	 * @extends {Create}
	 * @param {Object} data
	...
	class Sell extends Create {
		...
		get type() {
			return TransactionType.SELL_SHORT;
		}
		...
		_afterTransactionCreated(position, transactions, transaction) {
			...
			data.description = `Sell Short ${transaction.quantity.absolute().toFixed()} ${getDescriptionForInstrument(position.instrument)} @ ${transaction.trade.price.toFixed()}`;
