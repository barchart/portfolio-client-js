...
module.exports = (() => {
	...
	/**
	 * Creates a {@link TransactionType.DIVIDEND_STOCK} transaction.
	 *
	 * @public
	 * @extends {Create}
	 * @param {Object} data
	 ...
	 * @param {Decimal=} data.rate - The per-unit amount to be converted into new shares (if omitted, the "data.override" must be supplied).
	 * @param {Decimal=} data.price - The per-unit price used to convert the payment into new shares (if omitted, the "data.override" must be supplied).
	 * @param {Decimal=} data.open - The number of shares held at the time the payment is calculated (overrides the position as of the "effective" date).
	 * @param {Day=} data.effective - The day on which the number of shares held is determined (i.e. the ex-dividend date).
	 * @param {Decimal=} data.override - The number of shares distributed (if present, shares distributed is *not* calculated).
	 */
	class StockDividend extends Create {
		constructor(data) {
			super(data);
		...
		get type() {
			return TransactionType.DIVIDEND_STOCK;
		}
		...
		_createTransaction(position, transactions, data) {
			...
			if (data.override) {
				...
				quantity = open.multiply(data.rate).divide(data.price);
