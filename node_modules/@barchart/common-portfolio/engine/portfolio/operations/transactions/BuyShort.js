...
module.exports = (() => {
	...
	/**
	 * Creates a {@link TransactionType.BUY_SHORT} transaction.
	 *
	 * @public
	 * @extends {Create}
	 * @param {Object} data
	 ...
	 * @param {Decimal} data.price - The per-unit price of the purchase.
	 * @param {Decimal} data.quantity - The number of units purchased.
	 * @param {Decimal} data.fee - The fee charged to complete the transaction.
	 */
	class Buy extends Create {
		constructor(data) {
			super(data);
		...
		get type() {
			return TransactionType.BUY_SHORT;
		}

		_createTransaction(position, transactions, data) {
			const signedQuantity = data.quantity;
		...
		_afterTransactionCreated(position, transactions, transaction) {
			...
			data.description = `Buy To Cover ${transaction.quantity.toFixed()} ${getDescriptionForInstrument(position.instrument)} @ ${transaction.trade.price.toFixed()}`;
