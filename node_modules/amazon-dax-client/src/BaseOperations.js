...
module.exports = class BaseOperations {
  ...
  _getReturnHandler(tube, decoder, assembler) {
    // setup event listeners for all components and remove them when finished or an error occurs
    // when error occurs, depend on what kind of error, reset all tubes or just recycle back tube
    function cleanup() {
      tube.socket.removeAllListeners('error');
      decoder.removeAllListeners();
      assembler.removeAllListeners();
    }

    return new Promise((resolve, reject) => {
      assembler.on('data', (result) => {
        decoder.unpipe(assembler);
        tube.socket.unpipe(decoder);
    ...
    }).then((result) => { // no matter what comes, we need to cleanup then return the results
      cleanup();
      return result;
    }).catch((err) => {
      cleanup();
