  ...
  _writeInt(str) {
    if(typeof str === 'number') {
      ...
      if(str.length > 0 && str[0] === '-') {
        let b = new BigNumber(str);
        let buf = Buffer.from(Util.convertBase(b.neg().sub(1).toString(), 10, 16), 'hex');
        this._writeType(CborTypes.TYPE_NEGINT, buf);
      } else {
        let buf = Buffer.from(Util.convertBase(str, 10, 16), 'hex');
        this._writeType(CborTypes.TYPE_POSINT, buf);
      }
    } else {
      throw new DaxClientError('not supported type for Int: ' + typeof(str), DaxErrorCode.Encoder);
    }
  }
