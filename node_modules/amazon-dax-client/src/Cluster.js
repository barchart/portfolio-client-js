...
const IDLE_CONNECTION_REAP_DELAY_MS = 30000;
...
class Cluster {
  constructor(config, daxManufacturer, source) {
    ...
    this._clusterUpdateThreshold = config.clusterUpdateThreshold ? config.clusterUpdateThreshold : 125;
    this._clusterUpdateInterval = config.clusterUpdateInterval ? config.clusterUpdateInterval : 4000;
    this._connectTimeout = config.connectTimeout ? config.connectTimeout : 1000;
    this._requestTimeout = config.requestTimeout ? config.requestTimeout : 60000;
    this._healthCheckInterval = config.healthCheckInterval ? config.healthCheckInterval : 5000;
    this._healthCheckTimeout = config.healthCheckTimeout ? config.healthCheckTimeout : 1000;
    this._writeRetries = config.writeRetries ? config.writeRetries : 2;
    this._readRetries = config.readRetries ? config.readRetries : 2;
    this._maxRetryDelay = config.maxRetryDelay ? config.maxRetryDelay : 7000;
    this._threadKeepAlive = config.threadKeepAlive ? config.threadKeepAlive : 10000;
    ...
    this._region = config.region ? config.region : this._resolveRegion(); // FIXME default region
  ...
  _resolveRegion() { // read user's env, should add EC2 metadata in the future
    return process.env.AWS_REGION || process.env.AMAZON_REGION || 'us-east-1';
  }
  ...
  startup() {
    ...
    // Setup connection reaping, and cluster refresh jobs before trying
    // to bootstrap cluster in case the refresh() call fails.
    this._reapJob = setInterval(() => {
      try { // so far didn't see any chance to throw Error, maybe remove try/catch
        if(!this._closed) {
          this._reapIdleConnections();
        }
      } catch(err) {
        // maybe change it to logging
        console.error('caught exception during idle connection reaping:', err);
        console.error(err.stack);
      }
    }, IDLE_CONNECTION_REAP_DELAY_MS);

    let refreshIntervalMs = Math.min(5000, this._clusterUpdateInterval);
  ...
  newClient(host, port, session, tube, el) {
    if(tube) {
      // this need to be added to ClientTube method
      tube.socket.setTimeout(this._requestTimeout);
    }

    let pool = new SocketTubePool(host, port, this._credProvider, this._region, tube);
