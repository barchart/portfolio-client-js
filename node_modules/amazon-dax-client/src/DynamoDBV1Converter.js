...
class DynamoDBV1Converter {
  ...
  */
  ...
  static convertRequest(ddbRequest, type) {
    let request = Object.assign({}, ddbRequest);
    ...
    if(DynamoDBV1Converter.isV1Request(request, type)) {
      if(type === DaxMethodIds.batchGetItem && !DynamoDBV1Converter._isMapEmpty(request.RequestItems)) {
        request.RequestItems = JSON.parse(JSON.stringify(ddbRequest.RequestItems));
      }
  ...
  */
  static _handleExistsCriteria(attr, expectedAttrVal, eName, exprAttrValsMap) {
    if(expectedAttrVal.Exists || expectedAttrVal.Value) {
      let expr = '';
      if(expectedAttrVal.Exists) {
        // If Exists is true, a value must be provided
        if(expectedAttrVal.AttributeValueList) {
          throw new DaxClientError(
            'One or more parameter values were invalid: AttributeValueList can only be used with a ComparisonOperator for Attribute: ' + attr,
            DaxErrorCode.Validation, false);
        }
        if(!expectedAttrVal.Value) {
          throw new DaxClientError('One or more parameter values were invalid: Value must be provided when Exists is true for Attribute: ' + attr,
            DaxErrorCode.Validation, false);
        }
        ...
        if(expectedAttrVal.Value) {
          ...
          throw new DaxClientError('One or more parameter values were invalid: Value cannot be used when Exists is false for Attribute: ' + attr,
            DaxErrorCode.Validation, false);
        }
    ...
    throw new DaxClientError('One or more parameter values were invalid: Value must be provided when Exists is null for Attribute: ' + attr,
      DaxErrorCode.Validation, false);
  }
