...
class AutoconfSource {
  ...
  _resolveAddr(dests, index, callback) {
    if(index >= dests.length) {
      return callback(new DaxClientError('not able to resolve address', DaxErrorCode.NoRoute));
    }
  ...
  _pull(addrs, port, callback) {
    ...
    let count = 0;
    for(let ia of addrs) {
      this._pullFrom(ia, port).then((newCfg) => {
        if(success) {
          return;
        }
        ...
        if(++count === addrs.length) {
          return callback(new Error('not able to find configuration'));
        }
      }).catch((err) => {
        if(success) {
          return;
