...
const _AmazonDaxClient = AWS.util.inherit({
  constructor: function AmazonDaxClient(config, cluster) {
    ...
    this._cluster = cluster ? cluster : new Cluster(config, {createDaxClient: (pool, region, el) => {
      return new DaxClient(pool, region, el);
    }});
    ...
    this._readOperationsRetryHandler = new RetryHandler(this._cluster, this._maxRetryDelay);
    this._writeOperationsRetryHandler = new WriteOperationsRetryHandler(this._cluster, this._maxRetryDelay);
  },
...
class RetryHandler {
  constructor(cluster, retryDelay) {
  ...
  check(err) {
    ...
    if(err.codeSeq) {
      // this is internal logic avoid present to user
      delete err.codeSeq;
    }
    return err;
  }
  ...
  isRetryable(err) { // only some Dax/DDB error is not retryable, will be init when creating
