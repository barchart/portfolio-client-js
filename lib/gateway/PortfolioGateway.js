...
module.exports = (() => {
	...
	class PortfolioGateway extends Disposable {
		constructor(protocol, host, port, requestInterceptor) {
			...
			this._createTransactionEndpoint = EndpointBuilder.for('create-transaction', 'create transaction')
				...
				.withRequestInterceptor(RequestInterceptor.fromDelegate(createTransactionRequestInterceptor))
				.withRequestInterceptor(requestInterceptorToUse)
				.withResponseInterceptor(responseInterceptorForPortfolioDeserialization)
				.withErrorInterceptor(ErrorInterceptor.GENERAL)
				.endpoint;
		...
		createPortfolio(portfolio) {
			return Promise.resolve()
				.then(() => {
					...
					return Gateway.invoke(this._createPortfolioEndpoint, portfolio);
				});
		}
		...
		updatePortfolio(portfolio) {
			return Promise.resolve()
				.then(() => {
					...
					return Gateway.invoke(this._updatePortfolioEndpoint, portfolio);
				});
		}
		...
		deletePortfolio(portfolio) {
			return Promise.resolve()
				.then(() => {
					...
					return Gateway.invoke(this._deletePortfoliosEndpoint, { portfolio: portfolio });
				});
		}
		...
		deletePosition(portfolio, position) {
			return Promise.resolve()
				.then(() => {
					...
					return Gateway.invoke(this._deletePortfoliosEndpoint, { portfolio: portfolio, position: position });
				});
		}
		 ...
		 */
		createTransaction(portfolio, position, transaction) {
			return Promise.resolve()
				.then(() => {
					checkStart.call(this);
					...
					return Gateway.invoke(this._createTransactionEndpoint, { portfolio: portfolio, position: position || 'new', transaction: transaction });
				});
		}
	...
	const updatePortfolioRequestInterceptor = (request) => {
		return FailureReason.validateSchema(PortfolioSchema.UPDATE, request.data.portfolioData)
			.then(() => {
