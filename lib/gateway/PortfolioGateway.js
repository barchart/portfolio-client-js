...
module.exports = (() => {
	...
	class PortfolioGateway extends Disposable {
		constructor(protocol, host, port, environment, product) {
			...
			this._readSnapTradeAccountLinkEndpoint = EndpointBuilder.for('query-snaptrade-account-link', 'query-snaptrade-account-link')
				...
				.withRequestInterceptor(requestInterceptor)
				.withResponseInterceptor(responseInterceptorForSnapTradeAccountLink)
				.withErrorInterceptor(errorInterceptor)
				.endpoint;
		 ...
		 * @returns {Promise<Schema.SnapTradeBrokerage[]>}
		 */
		async disconnectSnapTradeBrokerage(connection) {
			checkStart.call(this);
		 ...
		 * @param {String} connection - The SnapTrade identifier for the brokerage connection.
		 * @param {String} account - The SnapTrade identifier for the brokerage account.
		 * @param {Boolean=} remove - Causes the linked portfolio to be deleteed.
		 * @returns {Promise<Schema.SnapTradeBrokerage[]>}
		 */
		async unlinkSnapTradeAccount(connection, account, remove) {
			...
			return Gateway.invoke(this._unlinkSnapTradeAccountEndpoint, { connection, account, remove });
		}
	...
	const responseInterceptorForSnapTradeBrokerages = ResponseInterceptor.fromDelegate((response, ignored) => {
		try {
			return deserializeSnapTradeBrokerages(response.data);
		} catch (e) {
			console.error('Error deserializing snaptrade brokerage data', e);
		}
	});

	const responseInterceptorForSnapTradeAccountLink = ResponseInterceptor.fromDelegate((response, ignored) => {
		try {
			return deserializeSnapTradeAccountLink(response.data);
		} catch (e) {
			console.error('Error deserializing snaptrade brokerage data', e);
		}
	});
	...
	function deserializeSnapTradeBrokerages(data) {
	...
	function deserializeSnapTradeAccountLink(data) {
		const status = data;
	 ...
	 * @param {SnapTradeLinkStatus} status
	 */
