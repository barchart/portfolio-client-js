...
module.exports = (() => {
	...
	class PortfolioGateway extends Disposable {
		constructor(protocol, host, port, environment, product) {
			...
			this._querySnapTradeAccountsEndpoint = EndpointBuilder.for('query-snaptrade-accounts', 'query-snaptrade-accounts')
				.withVerb(VerbType.GET)
				.withProtocol(protocolType)
				.withHost(host)
				.withPort(port)
				.withPathBuilder((pb) => {
					pb.withLiteralParameter('version', REST_API_VERSION)
						...
						.withVariableParameter('user', 'user', 'user', false)
		 ...
		 */
		async querySnapTradeAccounts(user) {
			checkStart.call(this);

			assert.argumentIsRequired(user, 'user', String);

			return Gateway.invoke(this._querySnapTradeAccountsEndpoint, { user });
		}
