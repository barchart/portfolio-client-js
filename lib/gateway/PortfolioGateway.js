...
module.exports = (() => {
	...
	const responseInterceptorForTransactionMutateDeserialization = ResponseInterceptor.fromDelegate((response, ignored) => {
		try {
			const saved = response.data.saved.map(p => JSON.parse(p, PositionSchema.CLIENT.schema.getReviver()));
			const deleted = response.data.deleted.map(p => JSON.parse(p, PositionSchema.CLIENT.schema.getReviver()));
