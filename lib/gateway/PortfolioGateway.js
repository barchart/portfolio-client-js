...
module.exports = (() => {
	...
	class PortfolioGateway extends Disposable {
		constructor(protocol, host, port, environment, product) {
			...
			this._readSnapTradeConnectionsEndpoint = EndpointBuilder.for('query-snaptrade-connections', 'query-snaptrade-connections')
				.withVerb(VerbType.GET)
				.withProtocol(protocolType)
				.withHost(host)
				.withPort(port)
		 ...
		 * @returns {Disposable}
		 */
		subscribePortfolio(portfolio, callback) {
			checkStart.call(this);
			...
			const poll = async () => {
				try {
					const portfolios = await this.readPortfolios(portfolio);

					if (is.array(portfolios) && portfolios.length === 1) {
		 ...
		 */
		async connectSnapTradeBroker(broker) {
			checkStart.call(this);

			assert.argumentIsRequired(broker, 'broker', String);

			return Gateway.invoke(this._connectSnapTradeBrokerEndpoint, { broker });
		}

		async reconnectSnapTradeBroker(connection) {
			checkStart.call(this);
		 ...
		 */
		async disconnectSnapTradeBroker(connection) {
			checkStart.call(this);

			assert.argumentIsRequired(connection, 'connection', String);

			return Gateway.invoke(this._disconnectSnapTradeBrokerEndpoint, { connection });
		}
		 ...
		 * @returns {Promise<Schema.SnapTradeConnection[]>}
		 */
		async readSnapTradeConnections() {
			checkStart.call(this);

			return Gateway.invoke(this._readSnapTradeConnectionsEndpoint, { });
		}
		 ...
		 * @returns {Promise<Schema.SnapTradeConnection[]>}
		 */
		async unlinkSnapTradeAccount(connection, account) {
			checkStart.call(this);

			assert.argumentIsRequired(connection, 'connection', String);
			assert.argumentIsRequired(account, 'account', String);

			return Gateway.invoke(this._unlinkSnapTradeAccountEndpoint, { connection, account });
		}
