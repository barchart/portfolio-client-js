...
const PortfolioUser = require('@barchart/watchlist-api-common/PortfolioUser');
...
module.exports = (() => {
	...
	class PortfolioGateway extends Disposable {
		...
		/**
		 * Retrieves the {@link PortfolioUser} from the remote server.
		 *
		 * @public
		 * @returns {Promise.<PortfolioUser>}
		 */
		readTransactions(portfolio, position) {
			...
			.then(() => {
				...
				return Gateway.invoke(this._readTransactionsEndpoint, { portfolio: portfolio, position: portfolio || '*' });
			});
		}
	...
	const requestInterceptorForSerialization = RequestInterceptor.fromDelegate((request, ignored) => {
		assert.argumentIsRequired(request.data, 'request.data', PortfolioUser, 'PortfolioUser');
	...
	const responseInterceptorForDeserialization = ResponseInterceptor.fromDelegate((response, ignored) => {
		return PortfolioUser.parse(response.data);
	});
