			...
			this._readTransactionsEndpoint = EndpointBuilder.for('read-transactions', 'read transactions')
				...
				.withQueryBuilder((qb) => {
					qb.withLiteralParameter('mode', 'mode', 'text')
						.withVariableParameter('sequence', 'sequence', 'sequence', true, x => x);
				})
				.withRequestInterceptor(RequestInterceptor.PLAIN_TEXT_RESPONSE)
				.withRequestInterceptor(requestInterceptor)
		...
		readTransactions(portfolio, position, sequence) {
			return Promise.resolve()
				.then(() => {
					...
					return Gateway.invoke(this._readTransactionsEndpoint, { portfolio: portfolio, position: position || '*', sequence: sequence });
				});
		}
