...
module.exports = (() => {
	...
	class PortfolioGateway extends Disposable {
		...
		/**
		 * Returns a promise which resolves as soon as the position's calculating status
		 * changes to false.
		 *
		 * @public
		 * @param {String} portfolio
					...
					setTimeout(() => {
						Gateway.invoke(this._readPositionsEndpoint, { portfolio: portfolio, position: position, includePreviousPrice: false })
							.then((positions) => {
								...
								if (is.object(p)) {
									if (is.object(p.system) && is.object(p.system) && is.boolean(p.system.calculating) && p.system.calculating) {
										scheduleCheck(delay + 1000);
									} else {
										resolveCallback(p);
									}
