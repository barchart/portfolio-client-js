...
module.exports = (() => {
	 ...
	 *
	 * @public
	 * @param {String} protocol - The protocol to use (either WS or WSS).
	 ...
	 * @param {String} wsProtocol - The host name of the Portfolio websocket service.
	 * @param {String} wsHost - The host name of the Portfolio websocket service.
	 * @param {Number} wsPort - The port number of the Portfolio websocket service.
	 *
	...
	class PortfolioGateway extends Disposable {
		...
		connectWebsocket(token, resolve) {
			try {
				...
				this._websocket = new WebSocket(`${this._wsProtocol}://${this._wsHost}:${this._wsPort}/?Auth=${token}`);
				...
				this._websocket.onclose = () => {
					setTimeout(this.setupWebsocket.bind(this, token), 1000);
				};

				resolve(true);

			} catch (e) {
				setTimeout(this.connectWebsocket.bind(this, token, resolve), 1000);
			}
		}
						...
						return this.setupWebsocket(token).then(() => {
							this._websocket.onmessage = (event) => {
								...
								if (data.clientId !== this._clientId) {
									const actionGroup = data.action.split('/')[0];
									const response = JSON.parse(data.response);

									switch (actionGroup) {
										case 'portfolio': {
											data.response = JSON.parse(data.response, PortfolioSchema.CLIENT.schema.getReviver());
											break;
										}
										case 'transaction': {
											data.response = deserializePositionMutateData(response, false);
											break;
										}
										case 'position': {
											data.response = deserializePositionMutateData(response, false);
		...
		static forDevelopment(requestInterceptor) {
			return Promise.resolve(requestInterceptor)
				.then((requestInterceptor) => {
					...
					return start(new PortfolioGateway('https', Configuration.developmentHost, 443, 'development', requestInterceptor, 'ws', Configuration.developmentHost, 3001));
				});
		}
		...
		static forStaging(requestInterceptor) {
			return Promise.resolve(requestInterceptor)
				.then((requestInterceptor) => {
					...
					return start(new PortfolioGateway('https', Configuration.stagingHost, 443, 'staging', requestInterceptor, 'ws', Configuration.stagingHost, 3001));
				});
		}
		...
		static forDemo(requestInterceptor) {
			return Promise.resolve(requestInterceptor)
				.then((requestInterceptor) => {
					...
					return start(new PortfolioGateway('https', Configuration.demoHost, 443, 'demo', requestInterceptor, 'ws', Configuration.demoHost, 3001));
				});
		}
		...
		static forProduction(requestInterceptor) {
			return Promise.resolve(requestInterceptor)
				.then((requestInterceptor) => {
					...
					return start(new PortfolioGateway('https', Configuration.productionHost, 443, 'production', requestInterceptor, 'ws', Configuration.productionHost, 3001));
				});
		}
		...
		static forAdmin(requestInterceptor) {
			return Promise.resolve(requestInterceptor)
				.then((requestInterceptor) => {
					...
					return start(new PortfolioGateway('https', Configuration.adminHost, 443, 'admin', requestInterceptor, 'ws', Configuration.adminHost, 3001));
				});
		}
