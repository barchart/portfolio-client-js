...
module.exports = (() => {
	...
	class JwtGateway extends Disposable {
		...
		static forDevelopment(userId, userLegacyId) {
			return start(new JwtGateway(forDevelopment(userId, userLegacyId), 60000));
		}
		 ...
		 * @returns {Promise.<JwtGateway>}
		 */
		static forStaging(externalRequestInterceptor) {
			return start(new JwtGateway(forStaging(externalRequestInterceptor), 60000));
		}
		 ...
		 * @returns {Promise.<RequestInterceptor>}
		 */
		static forStagingClient(externalRequestInterceptor) {
			return JwtGateway.forStaging(externalRequestInterceptor)
				.then((jwtGateway) => {
					return jwtGateway.toRequestInterceptor();
				});
		}
