...
gulp.task('embed-version', function () {
	...
	return gulp.src(['./lib/portfolio/index.js'])
		.pipe(replace(/(version:\s*')([0-9]+\.[0-9]+\.[0-9]+)(')/g, '$1' + version + '$3'))
		.pipe(gulp.dest('./lib/portfolio/'));
});

gulp.task('commit-changes', function () {
	return gulp.src(['./', './dist/', './package.json', './bower.json', './lib/portfolio/index.js'])
		.pipe(git.add())
...
gulp.task('build-browser', function () {
	return browserify('./lib/index.js', {standalone: 'Barchart.Portfolio'})
		.transform(partialify)
		.bundle()
		.pipe(source('barchart-portfolio-client.js'))
		.pipe(buffer())
		.pipe(gulp.dest('./dist'))
